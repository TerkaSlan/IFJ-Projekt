...
SHOULD_EQUAL("SubString1", strCmpCStr(substring1, "12"), 0);
strFree(substring1);
...
SHOULD_EQUAL("SubString3 (beginIndex + length = uiLength)", strCmpCStr(substring3, "56789"), 0);
strFree(substring3);

dtStr *substring4;
SHOULD_EQUAL("SubString4 (beginIndex + length > uiLength))", (substr(string, 6, 5, &substring4) == ERR_OTHER), 1);

dtStr *substring5;
SHOULD_EQUAL("SubString5 (length < 0)", (substr(string, 1, -1, &substring5) == ERR_OTHER), 1);
